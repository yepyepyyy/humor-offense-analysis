...
def create(x_train, y_train, x_val, y_val, params):
    ...
    if pretrained_embeddings == 'fasttext_english':
        custom_embeddings['fasttext_english'] = kerasutils.get_embedding_matrix(
            'fasttext_english', tokenizer, name, dataset
        )

    elif pretrained_embeddings == 'fasttext_chinese':
        custom_embeddings['fasttext_chinese'] = kerasutils.get_embedding_matrix(
            'fasttext_chinese', tokenizer, name, dataset
        )

    elif pretrained_embeddings == 'glove_english':
        custom_embeddings['glove_english'] = kerasutils.get_embedding_matrix(
            'glove_english', tokenizer, name, dataset
        )
    ...
    if (task_type == 'classification'):
        ...
        metric = keras.metrics.BinaryAccuracy(name="accuracy") if is_binary else keras.metrics.CategoricalAccuracy(
            name='accuracy')
    ...
    #                             different shapes
    neurons_per_layer = UMU.get_neurons_per_layer(shape, number_of_layers, first_neuron)
    ...
    if has_we:
        ...
        if (architecture == 'lstm'):
            layer_we = keras.layers.SpatialDropout1D(dropout_range)(layer_we)
            layer_we = keras.layers.LSTM(neurons_per_layer[0], dropout=dropout_range, recurrent_dropout=dropout_range)(
                layer_we)
        ...
        if (architecture == 'bilstm'):
            layer_we = keras.layers.SpatialDropout1D(dropout_range)(layer_we)
            layer_we = keras.layers.Bidirectional(
                keras.layers.LSTM(neurons_per_layer[0], dropout=dropout_range, recurrent_dropout=dropout_range))(
                layer_we)
        ...
        if (architecture == 'bigru'):
            layer_we = keras.layers.SpatialDropout1D(dropout_range)(layer_we)
            layer_we = keras.layers.Bidirectional(
                keras.layers.GRU(neurons_per_layer[0], dropout=dropout_range, return_sequences=False))(layer_we)
